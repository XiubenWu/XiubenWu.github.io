...
使用校园网时github经常抽风上不去，于是考虑使用手中的服务器做一个代理，看了一下tinyproxy比较轻量简单，就决定是你了。

# 安装

在Centos7及以前的版本可以使用yum安装，但是在Centos8目前还没有合适的包，于是进行源码安装：

项目地址为[https://github.com/tinyproxy/tinyproxy.github.io](https://github.com/tinyproxy/tinyproxy.github.io)，先clone下来：
...
完成之后，`tinyproxy/`下的`etc/`中存放了配置文件`tinyproxy.conf`，`tinyproxy/`下的`src/`中存放了配置文件`tinyproxy`。需要先对配置文件进行编辑，`tinyproxy.conf`内容：
...
- Port：端口，默认8888，可自定义。
- Allow：允许访问的ip，默认是本地的127.0.0.1，注释掉保证所有IP可以访问。
- BasicAuth：设置基本验证信息，注释掉则不需要。
...
```sh
/installdir/tinyproxy/src/tinyproxy  -c  /installdir/tinyproxy/etc/tinyproxy.conf
```
...
`设置--->网络和internet--->代理`，打开使用代理服务器，在其中填写ip地址和端口号。个别浏览器可能要进行代理配置。
